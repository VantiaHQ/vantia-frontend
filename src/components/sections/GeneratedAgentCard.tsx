...
import { Eye, Settings, CircleDollarSign, Trash2 } from "lucide-react"; // Added Eye, Removed Pencil
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
    ...
    <div className="container bg-[#070916] mx-auto pb-24">
        ...
        <Card className="max-w-3xl mx-auto shadow-lg bg-[url('/images/modular-agent.webp')] bg-cover bg-center relative rounded-lg drop-shadow-[0_-16px_32px_rgba(80,200,255,0.2)]"> {/* Whole card bg, glow shadow */}
            ...
            <CardHeader className="p-6 flex flex-col sm:flex-row items-center justify-between z-10"> {/* Removed bg image, added z-10 */}
                <Link href={agentUrl} target="_blank" rel="noopener noreferrer" className="text-primary font-headline font-bold text-white drop-shadow-[0_0_16px_rgba(80,200,255,0.3)] break-all hover:underline cursor-target text-2xl sm:text-3xl md:text-xl"> {/* Agent name as link, styled */}
                    {formattedName}
                </Link>
                
                {/* Buttons in header */}
                <div className="flex items-center gap-2 z-10 mt-4 sm:mt-0"> {/* Added mt-4 sm:mt-0 for spacing */}
                    <TooltipProvider>
                        <Tooltip>
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            ...
                                            <AlertDialogDescription>
                                                Esta acción eliminará la referencia a este agente de tu navegador. No podrás recuperarla a menos que generes el agente de nuevo.
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter className="gap-2">
                                            <AlertDialogCancel className="cursor-target rounded-full border border-gray-400 text-gray-400 hover:bg-white/20 hover:text-white">Cancelar</AlertDialogCancel>
                        <AlertDialogAction onClick={onClear} className="cursor-target rounded-full border border-red-600 text-red-600 hover:bg-red-600/20 hover:text-white bg-transparent">Eliminar</AlertDialogAction>
                                        </AlertDialogFooter>
                                    </AlertDialogContent>
                                </AlertDialog>
                            </TooltipTrigger>
